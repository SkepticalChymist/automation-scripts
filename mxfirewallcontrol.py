...
def getshardhost(p_apikey, p_orgid):
    ...
    time.sleep(API_EXEC_DELAY)
    try:
        r = requests.get('https://api.meraki.com/api/v0/organizations/%s/snmp' % p_orgid, headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
    except:
...
def gettemplatelist(p_apikey, p_shardhost, p_orgid):
    ...
    time.sleep(API_EXEC_DELAY)
    try:
        r = requests.get('https://api.meraki.com/api/v0/organizations/%s/configTemplates' % (p_orgid), headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
    except:
...
def readmxfwruleset(p_apikey, p_shardhost, p_nwid):
    ...
    time.sleep(API_EXEC_DELAY)
    try:
        r = requests.get('https://api.meraki.com/api/v0/networks/%s/l3FirewallRules' % (p_nwid), headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
    except:
...
def writemxfwruleset(p_apikey, p_shardhost, p_nwid, p_ruleset):
    ...
    time.sleep(API_EXEC_DELAY)
    try:
        r = requests.put('https://api.meraki.com/api/v0/networks/%s/l3FirewallRules/' % (p_nwid), data=json.dumps({'rules': p_ruleset}), headers={'X-Cisco-Meraki-API-Key': p_apikey, 'Content-Type': 'application/json'})
    except:
